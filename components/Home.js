  ...
  const movies = moviesData.map((data, i) => {
    ...
    let newOverview = () => {
      if (data.overview.length>250){
        return data.overview.substr(0,250) + '...'
      } else {
        return data.overview
      }
    }

    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={newOverview()} poster={`https://www.themoviedb.org/t/p/w440_and_h660_face/${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
